import {
  Table, Tbody, Td, Th, Thead, Tr, VStack, Text, Box,Spinner, Center, Flex
        ...
        <Table  size="sm">
          ...
          <Tbody bg="white">
            {data.ous.map((ou: any) =>
              <Tr key={ou.id}>
                <Td>{ou.name}</Td>
                {data.pes.map((pe: any) => <Td bg={colors(data.data[ou.id][pe.id]['indicator'])} textAlign="center" key={`${pe.id}${ou.id}`}>{data.data[ou.id][pe.id]['indicator']}</Td>)}
              </Tr>)}
          </Tbody>
        </Table>
      </VStack>}
