...
const Graph: FC<GraphOptions> = ({ indicator, filterBy, title, yAxisTitle }) => {
  const d2 = useD2()
  const periods$ = useStore(periods);
  const orgUnits$ = useStore(orgUnits);
  const maxLevel$ = useStore(maxLevel)
  const { data, isError, isLoading, error, isSuccess } = useAnalytics(d2, "vMfIVFcRWlu", "kHRn35W3Gq4", indicator, "rVZlkzOwWhi", "RgNQcLejbwX", `${orgUnits$};LEVEL-${maxLevel$}`, periods$, filterBy, 'SUM', true, true)
  return (
    <>
